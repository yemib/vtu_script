...
class UserResgisterController extends Controller
{
    /*
    |--------------------------------------------------------------------------
    | Register Controller
 ...
 $this->registered($request, $user)  ;
		...
		//now check the role to redirect  
		$role  = auth()->user()->role  ;
		
		if($role  ==  'Manager'){ 
		
        return redirect('/user/my_downlines');
		}
		
		  return redirect('/user/referees');
    ...
    protected function create(array $data)
    {
		...
		//now place the user under referral system 
	//&& $general->modules->referral_system
		//this will only work if  the session 
		if ($referBy ) {
			//check  if  is login in or not  
			//get the  user details of hthe person 
			$userd =  User::where('username'  , $referBy )->first() ;
			if(isset($userd->id) ) {
				$role  = $userd->role   ;
				if($role  ==  'Manager'   ){ 
				 $referUser =  null  ;  
				  ...
				  $referUser = User::where('username', $referBy)->first();
					...
					//again  check if  this user has manager  okay !!  
					
					if(isset($referUser->id)) { 
					if($referUser->manager  !=  0  ){
						 //get the 
						
						$manager = $referUser->manager ;
					}
        ...
        $user->country_code     =    "NG" ; //$data['country_code'];
        $user->mobile           = "+234".$data['mobile'];
        $user->address = [
            'country' => "NIGERIA",
            'city' => "  ",
            'zip' => "   ",
